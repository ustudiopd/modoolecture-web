# 모두의특강 웹 시스템 이식 명세서 v1.0

> **목적**: 이벤트 기반 질문 랭킹 및 아카이빙 플랫폼 이식 명세서  
> **대상**: 질문/답변 보드, 투표 시스템, AI 프롬프트 복사, NotebookLM 연동, 어드민 대시보드  
> **작성일**: 2025-01-XX
...
4. [질문/답변 보드 시스템](#4-질문답변-보드-시스템)
5. [투표 및 랭킹 시스템](#5-투표-및-랭킹-시스템)
...
- **에디터**: Novel.sh (Tiptap 기반) - 관리자 답변 작성용
- **데이터베이스**: Supabase (PostgreSQL)
- **인증**: Supabase Auth (선택적, 비로그인 사용자 허용)
...
1. **질문/답변 보드 시스템**
   - 이벤트(회차)별 질문 관리
   - 사용자 질문 작성 (경량 에디터 또는 텍스트 에어리어)
   - 전문가 답변 작성 (Novel.sh 에디터)
   - 카테고리별 분류 (Dev, Biz, Ethics 등)
   - 질문 제목 AI 자동 생성
...
- **UI 라이브러리**: 
  ...
  - Recharts (차트)
- **에디터**: 
  - Novel.sh (`novel` 패키지) - 관리자 답변 작성용
  - Tiptap Core (`@tiptap/core`, `@tiptap/react`)
  - Tiptap Extensions (starter-kit, markdown, html 등)
  - 사용자 질문 작성: 일반 Textarea 또는 경량 에디터
...
#### 다중 프로젝트 스키마 분리 전략
- **Prefix 기반 분리**: `modu_`, `uslab_`, `ustudio_` 등
...
CREATE TABLE modu_events (
  ...
  slug TEXT NOT NULL UNIQUE,             -- 예: 'ai-2025'
  title TEXT NOT NULL,                   -- 예: '2025 AI 결산'
  event_date TIMESTAMPTZ,                -- 행사 일시
  status TEXT DEFAULT 'open',            -- open, live, closed
  notebooklm_url TEXT,                   -- NotebookLM 공유 링크
...
CREATE TABLE modu_questions (
  ...
  title TEXT NOT NULL,                   -- 질문 요약/제목 (AI 자동 생성 가능)
  content TEXT NOT NULL,                 -- 질문 상세 내용
  category TEXT,                         -- 카테고리 태그 (Dev, Biz, Ethics 등)
  
  -- 답변 및 관리
  answer TEXT,                           -- 답변 내용 (Markdown)
  ...
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now(),
  author_id UUID REFERENCES auth.users(id) -- 로그인 사용자일 경우 (nullable)
);
...
CREATE TABLE modu_votes (
  ...
  session_id TEXT,                         -- 비로그인 유저 (브라우저 쿠키/IP 해시)
  created_at TIMESTAMPTZ DEFAULT now(),
...
#### 1. 질문 작성 페이지 (`/board/[event]/write`)

**주요 기능:**
- 이벤트 선택 (드롭다운)
- 카테고리 선택 (Dev, Biz, Ethics 등)
- 질문 내용 입력 (Textarea 또는 경량 에디터)
- 제목 자동 생성 옵션 (AI)
- 질문 제출
...
#### 1. 답변 작성 페이지 (`/admin/questions/[id]`)
...
- limit: number (기본값: 10)
- event_id: UUID (선택)
- category: string (선택: Dev, Biz, Ethics 등)
- sort: 'hot' | 'new' | 'answered' (기본값: 'new')
...
3. **Markdown 복사 테스트**
   - 질문 카드에서 복사 버튼 클릭
   - 클립보드에 Markdown 형식으로 복사되는지 확인

4. **관리자 답변 작성 테스트**
   - `/admin/login` 접속
   - 로그인
   - `/admin/questions/[id]` 접속
   - Novel.sh 에디터로 답변 작성
   - 답변 완료 처리
...
```sql
-- 카테고리는 자유 텍스트이므로 별도 테이블 불필요
-- 프론트엔드에서 드롭다운 옵션으로 관리
```
...
4. **트래킹 데이터 Retention**
   - 90일 자동 삭제 함수 (`modu_cleanup_old_tracking`) 정기 실행 필요
   - Vercel Cron 또는 Supabase Scheduled Triggers 사용

5. **세션 관리**
   - 비로그인 사용자의 세션 ID는 localStorage에 저장
   - 중복 투표 방지를 위한 세션 기반 체크 필수
